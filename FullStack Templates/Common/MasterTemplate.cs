...
namespace FullStack.Common
{
    ...
    <%@ Map Name="DbTypeCSharp" Src="DbType-CSharp" Description="DbType to CSharp Map" %>
        */
        ...
        {
            return string.Format("{0}Model", GetClassName(table));
        }

        public string GetManagerClassName(TableSchema table)
        {
            return string.Format("{0}Manager", GetClassName(table));
        }

        public string GetDataClassName(TableSchema table)
        {
            return string.Format("{0}Data", GetClassName(table));
        }
        ...
        public string GetProjectTypeGuid(FullStack.Common.ProjectType en)
        {
        ...
        public void AddAssemblyToProject(string projectPath, string assembly)
        {
            ...
            XNamespace ns = "http://schemas.microsoft.com/developer/msbuild/2003";
            var itemGroup = proj.Descendants(ns + "ItemGroup").FirstOrDefault(x => x.Descendants(ns + "Reference").Count() > 0);
            ...
            XElement item = new XElement(ns + "Reference", 
        ...
        /// <param name="parent">The name of the parent to group the file to, an 
        /// empty string if there is no parent file</param>
        public void AddProjectReferenceToProject(string projectPath, string projectToReferenceRelativePath, System.Guid projectToReferenceGuid, string projectToReferenceName)
        {
            XDocument proj = XDocument.Load(projectPath);

            XNamespace ns = "http://schemas.microsoft.com/developer/msbuild/2003";
            var itemGroup = proj.Descendants(ns + "ItemGroup").FirstOrDefault(x => x.Descendants(ns + "ProjectReference").Count() > 0);
            ...
            XElement item = new XElement(ns + "ProjectReference", 
                            new XAttribute("Include", projectToReferenceRelativePath));
        ...
        public void CleanupProjectFile(string projectPath)
        {
            ...
            //If the file is already listed, don't bother adding it again
             var foundElements = itemGroup.Descendants(ns + "ProjectReference").Where(x => x.Attribute("Include").Value.ToString() == string.Empty);
            var xElements = foundElements as IList<XElement> ?? foundElements.ToList();
            if (xElements.Any())
            {
                foreach (var foundElement in xElements)
                {
                    foundElement.Remove();
                }
            }
